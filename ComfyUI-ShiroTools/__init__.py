...
# ---------------------------------------------------------------------------
# SHIRO TOOLS: VIDEO LOOP & VIDEO COLOR HELPER FUNCTIONS
# ---------------------------------------------------------------------------
...
class ShiroAutoSeamLoopCut:
    ...
    def find_and_cut(self, images, start_search_from=0, start_search_to_0_auto=0, end_search_from_0_auto=0, end_search_to_0_auto=0, min_loop_frames=24, compare_window=4, preview_window=12, appearance_weight=1.0, motion_weight=2.0, edge_weight=0.5, max_eval_pairs=12000, use_luma=True):
...
class ShiroMotionDeStutterLoopCleaner:
    ...
    def clean(self, images, low_motion_ratio=0.35, protected_start_frames=2, protected_end_frames=2, max_frames_to_drop=3, max_consecutive_drops=2):
...
class ShiroBoundaryDeStutterLoopCleaner:
    ...
    def clean(self, images, low_motion_ratio=0.60, start_scan_frames=12, end_scan_frames=12, max_start_drop=5, max_end_drop=3, min_remaining_frames=16):
...
class ShiroLastFirstContextBridge:
    ...
    def _catmull_rom(self, p0, p1, p2, p3, t):
        t2 = t * t; t3 = t2 * t
        out = 0.5 * ((2.0 * p1) + (-p0 + p2) * t + (2.0 * p0 - 5.0 * p1 + 4.0 * p2 - p3) * t2 + (-p0 + 3.0 * p1 - 3.0 * p2 + p3) * t3)
    ...
    def append_bridge(self, images, context_before=3, context_after=3, bridge_frame_count=1, method="balanced_anti_first", max_first_progress=0.72, anti_first_strength=0.65, insert_mode="append"):
        if (not torch.is_tensor(images)) or images.dim() != 4: return (images, images[:0])
        n = int(images.shape[0])
        count = max(0, int(bridge_frame_count))
        if count <= 0 or n < 2: return (images.contiguous(), images[:0].contiguous())
        before = max(1, min(int(context_before), max(1, n - 1)))
        after = max(1, min(int(context_after), max(1, n - 1)))
        first = images[0].float(); last = images[-1].float()
        ...
        for i in range(1, count + 1):
            ...
            if method == "linear": b = linear
            elif method == "catmull_rom": b = cat
            elif method == "motion_extrapolated": b = mot
            elif method == "bezier": b = bez
            else:
                b = (cat * 0.40 + mot * 0.40 + linear * 0.20).clamp(0.0, 1.0)
                if method == "balanced_anti_first":
                    d_last = torch.mean((b - last) ** 2); d_first = torch.mean((b - first) ** 2)
        ...
        if insert_mode == "append": out = torch.cat((images, bridge_batch), dim=0).contiguous()
        elif insert_mode == "replace_last": out = torch.cat((images[:-1], bridge_batch[-1:].contiguous()), dim=0).contiguous()
        elif insert_mode == "replace_last_and_append": out = torch.cat((images[:-1], bridge_batch), dim=0).contiguous()
        elif insert_mode == "replace_first":
            if n <= 2: out = torch.cat((images, bridge_batch), dim=0).contiguous()
            else: out = torch.cat((images[1:], bridge_batch), dim=0).contiguous()
        else:
            if n <= 3: out = torch.cat((images[:-1], bridge_batch), dim=0).contiguous()
            else: out = torch.cat((images[1:-1], bridge_batch), dim=0).contiguous()
...
class ShiroAppendRIFEBridgeFrames:
    ...
    def _pick_candidates(self, candidates, original_images, count, pick_mode, min_distance_from_first=0.001, max_first_progress=0.72, first_penalty_strength=2.0):
        ...
        for i in range(total):
            f = candidates[i]
            d_last = self._mse_img(f, last); d_first = self._mse_img(f, first)
            denom = d_last + d_first
    ...
    def append(self, original_images, rife_context_result, bridge_frame_count=1, rife_multiplier=4, boundary_segment_index=2, pick_mode="anti_first_bias", insert_mode="replace_last_and_append", min_distance_from_first=0.001, max_first_progress=0.72, first_penalty_strength=2.0):
...
def _pair_image_similarity_small(a_small, b_small, color_compare=True):
    if bool(color_compare):
        if a_small.dim() == 2: dims = (0, 1)
        elif a_small.dim() == 3: dims = (1, 2) if a_small.shape[0] in (1, 3) else (0, 1)
        else: dims = tuple(range(a_small.dim()))
        mean_a = a_small.mean(dim=dims, keepdim=True); std_a = a_small.std(dim=dims, unbiased=False, keepdim=True).clamp(min=1e-6)
        mean_b = b_small.mean(dim=dims, keepdim=True); std_b = b_small.std(dim=dims, unbiased=False, keepdim=True).clamp(min=1e-6)
        b_small = ((b_small - mean_b) / std_b) * std_a + mean_a; b_small = b_small.clamp(0.0, 1.0)
...
class ShiroPreInterpolationLoopBoundaryCleaner:
    ...
    def clean(self, images, compare_pairs=2, remove_end_frames=2, remove_start_frames=0, color_compare=True, image_similarity=0.018, min_remaining_frames=8):
        ...
        if chosen_pair_count < compare_pairs:
            _shiro_green_log(f"[ShiroTools] PreInterpolationLoopBoundaryCleaner: compare_pairs={compare_pairs} requested, but only {chosen_pair_count} valid seam pair(s) were available for the selected candidate in a {n}-frame clip.")
        elif max_pairs_used < compare_pairs:
            _shiro_green_log(f"[ShiroTools] PreInterpolationLoopBoundaryCleaner: compare_pairs={compare_pairs} requested, but at most {max_pairs_used} valid seam pair(s) were available for some candidates in a {n}-frame clip.")
...
# ---------------------------------------------------------------------------
# SHIRO TOOLS: VIDEO / COLOR BALANCE
# ---------------------------------------------------------------------------
...
def _shiro_watermark_tensor_to_pil(watermark_image, watermark_mask=None):
    if watermark_image is None: return None
    if not torch.is_tensor(watermark_image): raise ValueError("watermark_image must be a ComfyUI IMAGE tensor when connected.")
    wm = watermark_image.detach().float().cpu()
    if wm.dim() == 4: wm = wm[0]
    if wm.dim() != 3: raise ValueError(f"Expected watermark_image shape [H,W,C] or [B,H,W,C], got {tuple(wm.shape)}.")
    if wm.shape[-1] not in (1, 3, 4): raise ValueError(f"Unsupported watermark_image channels: {wm.shape[-1]}. Expected 1, 3, or 4.")
    wm = wm.clamp(0, 1)
    arr = (wm.numpy() * 255.0).round().astype(np.uint8)
    if arr.shape[-1] == 4: base = Image.fromarray(arr, mode="RGBA")
    elif arr.shape[-1] == 3: base = Image.fromarray(arr, mode="RGB").convert("RGBA")
    else: base = Image.fromarray(arr[..., 0], mode="L").convert("RGBA")
    if watermark_mask is not None and torch.is_tensor(watermark_mask):
        ...
        if mask.dim() == 2:
            ...
            if alpha_img.size != base.size: alpha_img = alpha_img.resize(base.size, Image.Resampling.LANCZOS)
...
# ---------------------------------------------------------------------------
# SHIRO TOOLS: EXPORT (Mestre, A1111 Original e A1111 Escravo)
# ---------------------------------------------------------------------------
...
class ShiroExportGlobalConfig:
    ...
    def calculate_next_counter(self, folder_path):
        highest_number = 0
...
class ShiroSaveExportLinked:
    ...
    def save_image(self, images, export_config, endfix, format, quality, save_png_metadata, prompt=None, extra_pnginfo=None):
        full_path = export_config["full_path"]
        prefix = export_config["prefix"]
        counter = export_config["counter"]
        
        for idx, image in enumerate(images):
            i = 255. * image.cpu().numpy()
            img = Image.fromarray(np.clip(i, 0, 255).astype(np.uint8))
            
            filename = f"{prefix}{counter:05d}{endfix}.{format.lower()}"
            file_path = os.path.join(full_path, filename)
            
            # Rede de segurança: Só aumenta o contador se, por algum acaso louco,
            # exatamente a mesma imagem (com o mesmo endfix) já existir.
            ...
            # Prepara o +1 para a próxima imagem do lote (caso tenha gerado 4 imagens de uma vez, por ex)
            counter += 1 
    ...
    def save_image(self, images, output_folder, prefix, endfix, format, quality, save_png_metadata, disable_counter=False, prompt=None, extra_pnginfo=None):
        ...
        for idx, image in enumerate(images):
            ...
            if disable_counter:
                ...
                filename = f"{prefix}{endfix}{batch_suffix}.{format.lower()}"
                file_path = os.path.join(full_path, filename)
            else:
                filename = f"{prefix}{counter:05d}{endfix}.{format.lower()}"
                file_path = os.path.join(full_path, filename)
...
# ---------------------------------------------------------------------------
# SHIRO TOOLS: AUDIO (HELPER FUNCTIONS)
# ---------------------------------------------------------------------------
...
class ShiroTrimLeadingSilence:
    ...
    def trim(self, audio, threshold_db=-45.0, min_silence_ms=250, padding_ms=50):
        ...
        if active_indices.numel() == 0: return (audio,)
...
class ShiroLimitLongSilence:
    ...
    def limit(self, audio, threshold_db=-45.0, min_silence_ms=350, max_silence_ms=180):
        ...
        envelope = work.abs().amax(dim=tuple(range(work.dim() - 1))); silent = envelope <= threshold
        min_silence_samples = int(sample_rate * (int(min_silence_ms) / 1000.0))
...
class ShiroAudioSelector8:
    ...
    def check_lazy_status(self, audio_1, selected_slot=1, fallback_to_audio_1=True, **kwargs):
        ...
        if slot <= 1: return []
        name = f"audio_{slot}"
        if name in kwargs and kwargs.get(name) is None: return [name]
        return []
