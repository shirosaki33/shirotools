...
class ShiroLastFirstContextBridge:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "method": (["balanced", "motion_extrapolated", "catmull_rom", "bezier", "linear"], {"default": "balanced"}),
    ...
    def append_bridge(self, images, context_before=3, context_after=3, bridge_frame_count=1, method="balanced", insert_mode="append", enabled=True):
        if (not bool(enabled)) or (not torch.is_tensor(images)) or images.dim() != 4:
        ...
        for i in range(1, count + 1):
            ...
            if method == "linear":
                ...
                # Stable default: combines curve continuity, motion estimate, and a little direct blend.
...
class ShiroAppendRIFEBridgeFrames:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "pick_mode": (["balanced", "middle", "first", "last"], {"default": "balanced"}),
    ...
    def _pick_candidates(self, candidates, original_images, count, pick_mode):
        ...
        # balanced: choose candidate(s) whose visual progress is closest to evenly-spaced seam positions.
    ...
    def append(
        ...
        bridge = self._pick_candidates(candidates, original_images, count, pick_mode).to(dtype=original_images.dtype).contiguous()
